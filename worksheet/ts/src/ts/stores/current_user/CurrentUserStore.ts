...
export class CurrentUserStore {
    ...
    ((gapi.client as any).oauth2
      ...
      .then(
        (response) => {
          ...
          const self = this;
          Services.default.userSettingsQueryService
            .getUserSettings({id: currentUser.id})
            .then(
              (userSettings) => {
                self.setCurrentUser(
                  currentUser.replaceSettings(userSettings),
                  true
                );
              },
              (reason: Exception) => {
  ...
  setCurrentUserSettings(settings?: UserSettings) {
    ...
    if (settings) {
      ...
      currentUser!.services.userSettingsCommandService.putUserSettings({
        id: currentUser!.id,
  ...
  private setCurrentUser(
    ...
    if (typeof saveToLocalStorage === "undefined" || saveToLocalStorage) {
      localStorage.setItem(
        CurrentUserStore.CURRENT_USER_ITEM_KEY,
  ...
  private setCurrentUserFromLocalStorage() {
    ...
    const currentUser = CurrentUser.fromJsonObject(
