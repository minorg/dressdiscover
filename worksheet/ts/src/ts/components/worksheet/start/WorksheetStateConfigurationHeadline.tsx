...
export class WorksheetStateConfigurationHeadline extends React.Component<{ currentUserStore?: CurrentUserStore }> {
    render() {
        ...
        if (!currentUser) {
            return <React.Fragment>{localStorageText} (<Link to={Hrefs.login}>Login</Link> to change to a Google Sheet)</React.Fragment>;
        }
        ...
        return <React.Fragment>{text} (<Link to={Hrefs.userSettings}>Change</Link>)</React.Fragment>;
    }
}
