...
export class WorksheetStateEdit extends React.Component<
...
class WorksheetStateEditImpl extends React.Component<
...
> {
  constructor(props: WorksheetStateEditImplProps) {
    ...
    this.state = {
      description: this.props.worksheetState.text,
      selectedFeatureSetIds: this.props.worksheetState.selectedFeatureSetIds,
    };
  }
  ...
  onChangeDescription(e: any) {
    ...
    this.setState((prevState) =>
      Object.assign({}, prevState, {description: text})
    );
  }
  ...
  onToggleFeatureSet(featureSetId: WorksheetFeatureSetId) {
    this.setState((prevState) => {
      ...
      return Object.assign({}, prevState, {
        selectedFeatureSetIds: newSelectedFeatureIds,
      });
    });
  }
    ...
    return (
      ...
      >
        ...
        <Table className="table-bordered">
          ...
          <tbody>
            {worksheetState.worksheetDefinition.featureSets.map(
              (featureSetDefinition) => (
                <tr
                  className={classnames({
                    "feature-set": true,
                    selected: this.isFeatureSetSelected(
                      featureSetDefinition.id
                    ),
                  })}
                  key={featureSetDefinition.id.toString()}
                >
                  <td className="text-center">
                    <Button
                      active={this.isFeatureSetSelected(
                        featureSetDefinition.id
                      )}
                      color="secondary"
                      onClick={() =>
                        this.onToggleFeatureSet(featureSetDefinition.id)
                      }
                      size="lg"
                    >
                  ...
                  <td>
                    {featureSetDefinition.description ? (
                      <WorksheetDescriptionComponent
                        description={featureSetDefinition.description}
                      />
                    ) : null}
                  </td>
                  <td className="align-middle">
                    {featureSetDefinition.features.map(
                      (featureDefinition, featureDefinitionIndex) => (
                        <React.Fragment key={featureDefinition.id.toString()}>
                          <span
                            data-bind="text: displayName"
                            style={{
                              fontWeight:
                                featureDefinitionIndex % 2 === 0
                                  ? "bold"
                                  : "normal",
                            }}
                          >
                          ...
                          {featureDefinitionIndex + 1 <
                          featureSetDefinition.features.length
                            ? "  \u00b7"
                            : null}
                          &nbsp;
                        </React.Fragment>
  ...
  save() {
    ...
    currentUserStore!.currentUserServices.worksheetStateCommandService.putWorksheetState(
      {state: worksheetState.toWorksheetState()}
    );
  }
}
