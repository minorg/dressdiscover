...
export class TextWorksheetStateExporter implements StringWorksheetStateExporter {
    export(worksheetDefinition: WorksheetDefinition, worksheetStates: WorksheetState[]): string {
        const worksheetDefinitionWrapper = new WorksheetDefinitionWrapper(worksheetDefinition);
        ...
        for (const worksheetState of worksheetStates) {
            ...
            for (const featureSetState of worksheetState.featureSets) {
                const featureSetDefinition = worksheetDefinitionWrapper.featureSetById(featureSetState.id);
                const clauses: string[] = [];
                for (const featureState of featureSetState.features) {
                    if (!featureState.selectedValueIds) {
                        continue;
                    }
                    const clause = featureState.selectedValueIds.map(featureValueId => worksheetDefinitionWrapper.featureValueById(featureValueId).displayName);
                    if (clause.length > 0) {
                        clauses.push(clause.join(", "));
                    }
                }
                if (clauses.length > 0) {
                    paragraphs.push(featureSetDefinition.displayName + ": " + clauses.join("; "));
                }
            }
        }
