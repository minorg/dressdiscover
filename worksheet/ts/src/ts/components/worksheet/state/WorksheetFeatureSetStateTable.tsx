...
export class WorksheetFeatureSetStateTable extends React.Component<Props> {
  render() {
    const includeFeatureDescriptions =
      ...
      this.props.featureSetDefinition.features.some(
        (featureDefinition) => !!featureDefinition.description
      );

    const rows = this.props.featureSetDefinition.features.map(
      (featureDefinition) => {
        ...
        const featureStateMark: WorksheetStateMark = {
        ...
        return new WorksheetFeatureSetStateTableRow(
          featureDefinition,
          featureStateMark,
          this.props.worksheetState.worksheetDefinition,
          featureState
        );
      }
    );
              ...
              <tr key={row.featureDefinition.id.toString()}>
                <td className="text-center w-25">
                  <Link
                    ...
                    to={Hrefs.worksheetState(row.featureStateMark)}
                  >
                    {row.featureDefinition.displayName}
                  </Link>
                </td>
                {includeFeatureDescriptions ? (
                  <td>
                    {row.featureDefinition.description ? (
                      <WorksheetDescriptionComponent
                        description={row.featureDefinition.description}
                      />
                    ) : null}
                  </td>
                ) : null}
...
class WorksheetFeatureSetStateTableRow {
  constructor(
    ...
    readonly featureStateMark: WorksheetStateMark,
