...
export class WorksheetFeatureDefinition {
  constructor(kwds: {
    ...
    valueIds: WorksheetFeatureValueId[];
    description?: WorksheetDescription;
  }) {
    this.displayNameEn = WorksheetFeatureDefinition.validateDisplayNameEn(
      kwds.displayNameEn
    );
  ...
  public deepCopy(): WorksheetFeatureDefinition {
    return new WorksheetFeatureDefinition({
      ...
      id: this.id,
      valueIds: this.valueIds.map((value0) => value0),
  ...
  public static fromThryftJsonObject(json: any): WorksheetFeatureDefinition {
    ...
    for (const fieldName in json) {
      if (fieldName === "display_name_en") {
        ...
        valueIds = json[fieldName].map((element: any) =>
  ...
  private static validateValueIds(
    valueIds: WorksheetFeatureValueId[]
    ...
    if (valueIds.length < 1) {
      throw new RangeError(
        "expected len(valueIds) to be >= 1, was " + valueIds.length
      );
    }
    return valueIds;
  }
