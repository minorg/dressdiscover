...
export class WorksheetDefinition {
  constructor(kwds: {
    featureSets: WorksheetFeatureSetDefinition[];
    featureValues: WorksheetFeatureValueDefinition[];
    features: WorksheetFeatureDefinition[];
  }) {
    this.featureSets = WorksheetDefinition.validateFeatureSets(
      kwds.featureSets
    );
    this.featureValues = WorksheetDefinition.validateFeatureValues(
      kwds.featureValues
    );
  ...
  public deepCopy(): WorksheetDefinition {
    return new WorksheetDefinition({
      ...
      featureValues: this.featureValues.map((value0) => value0.deepCopy()),
      features: this.features.map((value0) => value0.deepCopy()),
    });
  }
  ...
  public equals(other: WorksheetDefinition): boolean {
    if (
      !((
        left: WorksheetFeatureSetDefinition[],
        right: WorksheetFeatureSetDefinition[]
      ): boolean => {
    ...
    if (
      !((
        left: WorksheetFeatureValueDefinition[],
        right: WorksheetFeatureValueDefinition[]
      ): boolean => {
        if (left.length !== right.length) {
          return false;
        }
        ...
        return true;
      })(this.featureValues, other.featureValues)
    ) {
      return false;
    }
    ...
    if (
      !((
        left: WorksheetFeatureDefinition[],
        right: WorksheetFeatureDefinition[]
      ): boolean => {
        if (left.length !== right.length) {
          return false;
        }
    ...
    for (const fieldName in json) {
      if (fieldName === "feature_sets") {
        featureSets = json[fieldName].map((element: any) =>
          WorksheetFeatureSetDefinition.fromThryftJsonObject(element)
        );
      } else if (fieldName === "feature_values") {
        featureValues = json[fieldName].map((element: any) =>
          WorksheetFeatureValueDefinition.fromThryftJsonObject(element)
        );
      } else if (fieldName === "features") {
        features = json[fieldName].map((element: any) =>
          WorksheetFeatureDefinition.fromThryftJsonObject(element)
        );
      }
    }
    if (featureSets == null) {
  ...
  public toJsonObject(): {
    ...
    feature_values: Array<{
      ...
      image:
        | {
            full_size_url:
              | {absolute: string | undefined; relative: string | undefined}
              | undefined;
            rights: {
              author: string;
              license: {nickname: string; statement: string; uri: string};
    ...
    return {
      ...
      feature_values: this.featureValues.map((inElement) =>
        inElement.toJsonObject()
      ),
  ...
  public toString(): string {
    return (
      "WorksheetDefinition(" + JSON.stringify(this.toThryftJsonObject()) + ")"
    );
  }
  ...
  public toThryftJsonObject(): {
    ...
    return {
      ...
      features: this.features.map((inElement) =>
        inElement.toThryftJsonObject()
      ),
    };
  }
  ...
  private static validateFeatureSets(
    ...
    if (featureSets.length < 1) {
      throw new RangeError(
        "expected len(featureSets) to be >= 1, was " + featureSets.length
      );
    }
    return featureSets;
  }

  private static validateFeatureValues(
    featureValues: WorksheetFeatureValueDefinition[]
    ...
    if (featureValues.length < 1) {
      throw new RangeError(
        "expected len(featureValues) to be >= 1, was " + featureValues.length
      );
    }
  ...
  private static validateFeatures(
    ...
    if (features.length < 1) {
      throw new RangeError(
        "expected len(features) to be >= 1, was " + features.length
      );
    }
    return features;
  }
