...
export class WorksheetFeatureSetDefinition {
  constructor(kwds: {
    ...
    featureIds: WorksheetFeatureId[];
    id: WorksheetFeatureSetId;
    description?: WorksheetDescription;
  }) {
    this.displayNameEn = WorksheetFeatureSetDefinition.validateDisplayNameEn(
      kwds.displayNameEn
    );
    this.featureIds = WorksheetFeatureSetDefinition.validateFeatureIds(
      kwds.featureIds
    );
  ...
  public deepCopy(): WorksheetFeatureSetDefinition {
    return new WorksheetFeatureSetDefinition({
      displayNameEn: this.displayNameEn,
      featureIds: this.featureIds.map((value0) => value0),
      id: this.id,
      description: this.description ? this.description.deepCopy() : undefined,
    });
  }
  ...
  public equals(other: WorksheetFeatureSetDefinition): boolean {
    ...
    if (
      !((left: WorksheetFeatureId[], right: WorksheetFeatureId[]): boolean => {
        ...
        return true;
      })(this.featureIds, other.featureIds)
    ) {
      return false;
    }
    ...
    if (
      ...
      !(typeof other.description === "undefined")
        ? (this.description as WorksheetDescription).equals(
            other.description as WorksheetDescription
          )
        : typeof this.description === "undefined" &&
          typeof other.description === "undefined")
    ) {
  ...
  public static fromThryftJsonObject(json: any): WorksheetFeatureSetDefinition {
    ...
    for (const fieldName in json) {
      if (fieldName === "display_name_en") {
        ...
        featureIds = json[fieldName].map((element: any) =>
          WorksheetFeatureId.parse(element)
        );
      } else if (fieldName === "id") {
        id = WorksheetFeatureSetId.parse(json[fieldName]);
      } else if (fieldName === "description") {
        description = WorksheetDescription.fromThryftJsonObject(
          json[fieldName]
        );
      }
    }
  ...
  public toJsonObject(): {
    ...
    return {
      description:
        this.description != null ? this.description.toJsonObject() : undefined,
    ...
    return {
      description:
        this.description != null
          ? this.description.toThryftJsonObject()
          : undefined,
      display_name_en: this.displayNameEn,
      feature_ids: this.featureIds.map((inElement) => inElement.toString()),
      id: this.id.toString(),
  ...
  private static validateDisplayNameEn(displayNameEn: string): string {
    ...
    if (displayNameEn.length < 1) {
      throw new RangeError(
        "expected len(displayNameEn) to be >= 1, was " + displayNameEn.length
      );
    }
    return displayNameEn;
  }

  private static validateFeatureIds(
    featureIds: WorksheetFeatureId[]
  ): WorksheetFeatureId[] {
    if (featureIds == null) {
    ...
    if (featureIds.length < 1) {
      throw new RangeError(
        "expected len(featureIds) to be >= 1, was " + featureIds.length
      );
    }
    return featureIds;
  }
