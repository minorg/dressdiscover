export class WorksheetFeatureValueImageUrl {
  constructor(kwds?: {absolute?: string; relative?: string}) {
    ...
    this.relative = WorksheetFeatureValueImageUrl.validateRelative(
      kwds.relative
    );
  }

  public deepCopy(): WorksheetFeatureValueImageUrl {
    return new WorksheetFeatureValueImageUrl({
  ...
  private static validateRelative(
    relative: string | undefined
  ): string | undefined {
    if (relative != null) {
      ...
      if (relative.length < 1) {
        throw new RangeError(
          "expected len(relative) to be >= 1, was " + relative.length
        );
      }
    }
    return relative;
