...
export class WorksheetRights {
  ...
  public deepCopy(): WorksheetRights {
    return new WorksheetRights({
      author: this.author,
      license: this.license.deepCopy(),
      source: this.source.deepCopy(),
    });
  }
  ...
  private static validateAuthor(author: string): string {
    ...
    if (author.length < 1) {
      throw new RangeError(
        "expected len(author) to be >= 1, was " + author.length
      );
    }
    return author;
  }
