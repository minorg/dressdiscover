...
export class WorksheetFeatureState {
  constructor(kwds: {
    id: WorksheetFeatureId;
    selectedValueIds?: WorksheetFeatureValueId[];
    text?: string;
  }) {
    this.idPrivate = WorksheetFeatureState.validateId(kwds.id);
    this.selectedValueIdsPrivate = WorksheetFeatureState.validateSelectedValueIds(
      kwds.selectedValueIds
    );
  ...
  set selectedValueIds(
    selectedValueIds: WorksheetFeatureValueId[] | undefined
  ) {
    this.selectedValueIdsPrivate = WorksheetFeatureState.validateSelectedValueIds(
      selectedValueIds
    );
  ...
  public deepCopy(): WorksheetFeatureState {
    return new WorksheetFeatureState({
      id: this.id,
      selectedValueIds: this.selectedValueIds
        ? this.selectedValueIds.map((value0) => value0)
        : undefined,
      text: this.text,
    });
  }
  ...
  public equals(other: WorksheetFeatureState): boolean {
    ...
    if (
      !(!(typeof this.selectedValueIds === "undefined") &&
      !(typeof other.selectedValueIds === "undefined")
        ? ((
            left: WorksheetFeatureValueId[],
            right: WorksheetFeatureValueId[]
          ): boolean => {
            ...
            this.selectedValueIds as WorksheetFeatureValueId[],
            other.selectedValueIds as WorksheetFeatureValueId[]
          )
        : typeof this.selectedValueIds === "undefined" &&
          typeof other.selectedValueIds === "undefined")
    ) {
  ...
  public static fromThryftJsonObject(json: any): WorksheetFeatureState {
    ...
    for (const fieldName in json) {
      if (fieldName === "id") {
        ...
        selectedValueIds = json[fieldName].map((element: any) =>
          WorksheetFeatureValueId.parse(element)
        );
  ...
  public toJsonObject(): {
    ...
    return {
      id: this.id.toString(),
      selected_value_ids:
        this.selectedValueIds != null
          ? this.selectedValueIds.map((inElement) => inElement.toString())
          : undefined,
  ...
  public toString(): string {
    return (
      "WorksheetFeatureState(" + JSON.stringify(this.toThryftJsonObject()) + ")"
    );
  }
  ...
  private static validateSelectedValueIds(
    selectedValueIds: WorksheetFeatureValueId[] | undefined
  ): WorksheetFeatureValueId[] | undefined {
    if (selectedValueIds != null) {
      if (selectedValueIds.length < 1) {
        throw new RangeError(
          "expected len(selectedValueIds) to be >= 1, was " +
