...
class DeleteExistingFileConfirmationModal extends React.Component<
  {
    existingFile: gapi.client.drive.File;
    onCancel: () => void;
    onConfirm: () => void;
  },
  {isOpen: boolean}
  ...
  render() {
    ...
    return (
      <Modal isOpen={this.state.isOpen}>
        <ModalHeader>
          Are you sure you want to delete the sheet '{existingFile.name}'?
        </ModalHeader>
        <ModalBody>
          <Container fluid>
            <Row>
              <Col className="text-center" md="6">
                <Button color="danger" onClick={onConfirmWrapper}>
                  Delete
                </Button>
              </Col>
              <Col className="text-center" md="6">
                <Button color="default" onClick={onCancelWrapper}>
                  Cancel
                </Button>
              </Col>
            </Row>
...
export class GoogleSheetsWorksheetStateConfigurationComponent extends React.Component<
  Props,
  State
> {
  constructor(props: Props) {
    ...
    this.onClickCancelDeleteExistingFile = this.onClickCancelDeleteExistingFile.bind(
      this
    );
    this.onClickConfirmDeleteExistingFile = this.onClickConfirmDeleteExistingFile.bind(
      this
    );
  ...
  onClickConfirmDeleteExistingFile() {
    ...
    gapi.client.drive.files.delete({fileId: file.id as string}).then(
      (response) => {
        this.refreshExistingFiles();
      },
      (reason: any) => {
        this.setState((prevState) => ({
          deletingExistingFile: undefined,
          exception: convertGapiErrorToException(reason),
          newSheetName: prevState.newSheetName,
        }));
      }
    );
  }
    ...
    if (exception) {
      ...
      return (
        <DeleteExistingFileConfirmationModal
          existingFile={deletingExistingFile}
          onCancel={this.onClickCancelDeleteExistingFile}
          onConfirm={this.onClickConfirmDeleteExistingFile}
        />
      );
                      ...
                      <tbody>
                        {existingFiles.map((existingFile) => {
                          const selected: boolean =
                            ...
                            googleSheetsWorksheetStateConfiguration.spreadsheetId ===
                              existingFile.id;
                          const tdStyle:
                            | React.CSSProperties
                            | undefined = selected
                            ...
                            <tr key={existingFile.id}>
                              ...
                              >
                                <Button
                                  ...
                                  color="secondary"
                                  onClick={() =>
                                    this.onClickExistingFile(existingFile)
                                  }
                                >
                                  {existingFile.name}
                                </Button>
                              ...
                              >
                                <Button
                                  onClick={() =>
                                    this.onClickDeleteExistingFile(existingFile)
                                  }
        ...
        <Row>
          <Col className="text-center" md="6">
            <Form inline>
              <Input
                type="text"
                onChange={this.onChangeNewSheetName}
              ...
              <Button
                className="ml-4"
                disabled={!this.state.newSheetName.length}
                color="primary"
                onClick={this.onClickNewSheetButton}
              >
                Create a new Sheet
              </Button>
            </Form>
