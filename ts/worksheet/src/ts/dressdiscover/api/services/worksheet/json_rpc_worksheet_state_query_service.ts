...
export class JsonRpcWorksheetStateQueryService implements WorksheetStateQueryService {
    ...
    getWorksheetStateAsync(kwds: {id: WorksheetStateId, error: (errorKwds: {textStatus: string, errorThrown: any, [index: string]: any}) => any, success: (returnValue: WorksheetState) => void}): void {
        ...
        $.ajax({
            ...
            success: (data, textStatus, jqXHR) => {
                ...
                let __error: Error | undefined | IoException | NoSuchWorksheetStateException = undefined;
    ...
    getWorksheetStateSync(kwds: {id: WorksheetStateId}): WorksheetState {
        ...
        let __error: Error | undefined | IoException | NoSuchWorksheetStateException = undefined;
        let __returnValue: WorksheetState | undefined = undefined;
    ...
    getWorksheetStateIdsAsync(kwds: {error: (errorKwds: {textStatus: string, errorThrown: any, [index: string]: any}) => any, success: (returnValue: WorksheetStateId[]) => void}): void {
        $.ajax({
            ...
            success: (data, textStatus, jqXHR) => {
                ...
                if (typeof data.result !== "undefined") {
                    kwds.success(function(json: any[]): WorksheetStateId[] { let sequence: WorksheetStateId[] = []; for (let i = 0; i < json.length; i++) { sequence.push(WorksheetStateId.parse(json[i])); } return sequence; }(data.result));
                    return;
                }
    ...
    getWorksheetStateIdsSync(): WorksheetStateId[] {
        let __error: Error | undefined | IoException = undefined;
        let __returnValue: WorksheetStateId[] | undefined = undefined;
        ...
        $.ajax({
            ...
            success: (data) => {
                if (typeof data.result !== "undefined") {
                    __returnValue = function(json: any[]): WorksheetStateId[] { let sequence: WorksheetStateId[] = []; for (let i = 0; i < json.length; i++) { sequence.push(WorksheetStateId.parse(json[i])); } return sequence; }(data.result);
                } else {
                    if (data.error["@class"] && data.error.data) {
                        const __error_class = data.error["@class"];
                        if (__error_class == 'dressdiscover.api.services.io_exception.IoException') {
