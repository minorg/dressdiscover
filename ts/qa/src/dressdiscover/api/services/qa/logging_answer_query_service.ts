...
export class LoggingAnswerQueryService implements AnswerQueryService {
    ...
    getAnswersAsync(kwds: {objectId: QaObjectId, questionSetId: QuestionSetId, questionIds?: QuestionId[], userId?: QaUserId, error?: (jqXHR: JQueryXHR | null, textStatus: string, errorThrown: string | null) => any, success?: (returnValue: Answer[]) => void}): void {
        this.delegate.getAnswersAsync({objectId: kwds.objectId, questionSetId: kwds.questionSetId, questionIds: kwds.questionIds, userId: kwds.userId, error: (jqXHR: JQueryXHR | null, textStatus: string, errorThrown: string | null) => { console.warn("getAnswersAsync({", "objectId: ", kwds.objectId, "questionSetId: ", kwds.questionSetId, "questionIds: ", kwds.questionIds, "userId: ", kwds.userId, "}) -> ", textStatus); console.warn(errorThrown); if (kwds.error) { kwds.error(jqXHR, textStatus, errorThrown); } }, success: (returnValue: Answer[]) => { console.debug("getAnswersSync({", "objectId: ", kwds.objectId, "questionSetId: ", kwds.questionSetId, "questionIds: ", kwds.questionIds, "userId: ", kwds.userId, "}) -> ", returnValue); if (kwds.success) { kwds.success(returnValue); } }});
    }
