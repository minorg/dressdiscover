export class WorksheetRightsLicense {
    ...
    public deepCopy(): WorksheetRightsLicense {
        return new WorksheetRightsLicense({ nickname: this.nickname, statement: this.statement, uri: this.uri });
    }
    ...
    public toString(): string {
        return "WorksheetRightsLicense(" + JSON.stringify(this.toThryftJsonObject()) + ")";
    }

    public toThryftJsonObject(): {nickname: string, statement: string, uri: string} {
        return {
            nickname: this.nickname,
            statement: this.statement,
            uri: this.uri
    ...
    private static validateStatement(statement: string): string {
        ...
        if (statement.length < 1) {
            throw new RangeError("expected len(statement) to be >= 1, was " + statement.length);
        }
        return statement;
    }
