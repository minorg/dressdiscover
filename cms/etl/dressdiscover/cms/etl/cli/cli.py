...
class Cli:
    ...
    def __import_pipeline_class(self, pipeline: str) -> _Pipeline:
        pipeline_module = import_module(pipeline, _Pipeline.__module__.rsplit(".", 1)[0])
    ...
    def main(self):
        ...
        storage = FilePipelineStorage(data_dir_path)

        pipeline = self.__instantiate_pipeline(args, pipeline_class, storage=storage)
        ...
        ttl_file_path = pipeline.transformer.transform(force=force_transform, **extract_kwds)
