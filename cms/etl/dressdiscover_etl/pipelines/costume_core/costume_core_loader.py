    ...
    def load(self, *, force: bool, models: Generator[_Model, None, None]):
        ...
        with open(models_dir_path / "costume_core_predicates.py", "w+", encoding="ascii") as predicates_py_file:
            ...
            for predicate in predicates:
                print(f"{predicate.id} = {dataclasses.replace(predicate, terms=None)}", file=predicates_py_file)
        ...
        with open(models_dir_path / "costume_core_terms.py", "w+", encoding="ascii") as terms_py_file:
            ...
            terms = tuple(filter(lambda model: isinstance(model, CostumeCoreTerm), models_tuple))
            for term in terms:
