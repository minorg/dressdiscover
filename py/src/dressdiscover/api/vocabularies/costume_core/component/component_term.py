...
class ComponentTerm(object):
    class Builder(object):
        ...
        def build(self):
            return ComponentTerm(text=self.__text, vocab_ref=self.__vocab_ref)
        ...
        def update(self, component_term):
            '''
            :type text: str
            :type vocab_ref: dressdiscover.api.models.vocab_ref.VocabRef or None
            '''

            if isinstance(component_term, ComponentTerm):
                self.set_text(component_term.text)
                self.set_vocab_ref(component_term.vocab_ref)
            elif isinstance(component_term, dict):
                for key, value in component_term.iteritems():
                    getattr(self, 'set_' + key)(value)
            else:
                raise TypeError(component_term)
            return self
    ...
    def __repr__(self):
        ...
        return 'ComponentTerm(' + ', '.join(field_reprs) + ')'
    ...
    def write(self, oprot):
        ...
        oprot.write_struct_begin('ComponentTerm')
