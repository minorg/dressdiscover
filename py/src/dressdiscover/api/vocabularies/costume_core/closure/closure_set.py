...
import __builtin__
import dressdiscover.api.vocabularies.costume_core.closure.closure


class ClosureSet(object):
    class Builder(object):
        def __init__(
            ...
            '''
            :type elements: tuple(dressdiscover.api.vocabularies.costume_core.closure.closure.Closure)
        ...
        def build(self):
            return ClosureSet(elements=self.__elements, display=self.__display, notes=self.__notes)
        ...
        def elements(self):
            '''
            :rtype: tuple(dressdiscover.api.vocabularies.costume_core.closure.closure.Closure)
            '''
        ...
        def set_elements(self, elements):
            ...
            if not (isinstance(elements, tuple) and len(list(ifilterfalse(lambda _: isinstance(_, dressdiscover.api.vocabularies.costume_core.closure.closure.Closure), elements))) == 0):
                raise TypeError("expected elements to be a tuple(dressdiscover.api.vocabularies.costume_core.closure.closure.Closure) but it is a %s" % getattr(__builtin__, 'type')(elements))
            if len(elements) < 1:
        ...
        def update(self, closure_set):
            ...
            if isinstance(closure_set, ClosureSet):
                self.set_elements(closure_set.elements)
                self.set_display(closure_set.display)
                self.set_notes(closure_set.notes)
            elif isinstance(closure_set, dict):
                for key, value in closure_set.iteritems():
                    getattr(self, 'set_' + key)(value)
            else:
                raise TypeError(closure_set)
            return self
    ...
    def __repr__(self):
        ...
        return 'ClosureSet(' + ', '.join(field_reprs) + ')'
    ...
    def read(cls, iprot):
        ...
        while True:
            ...
            if ifield_type == 0: # STOP
                ...
                init_kwds['elements'] = tuple([dressdiscover.api.vocabularies.costume_core.closure.closure.Closure.read(iprot) for _ in xrange(iprot.read_list_begin()[1])] + (iprot.read_list_end() is None and []))
    ...
    def replace(
        ...
        :type elements: tuple(dressdiscover.api.vocabularies.costume_core.closure.closure.Closure) or None
        :type display: str or None
        :type notes: str or None
        :rtype: dressdiscover.api.vocabularies.costume_core.closure.closure_set.ClosureSet
        '''
    ...
    def write(self, oprot):
        ...
        oprot.write_struct_begin('ClosureSet')
