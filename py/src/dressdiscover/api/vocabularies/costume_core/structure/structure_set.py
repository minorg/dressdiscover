...
import __builtin__
import dressdiscover.api.vocabularies.costume_core.structure.structure
...
class StructureSet(object):
    class Builder(object):
        def __init__(
            ...
            '''
            :type elements: tuple(dressdiscover.api.vocabularies.costume_core.structure.structure.Structure)
        ...
        def build(self):
            return StructureSet(elements=self.__elements, display=self.__display, notes=self.__notes)
        ...
        def elements(self):
            '''
            :rtype: tuple(dressdiscover.api.vocabularies.costume_core.structure.structure.Structure)
            '''
        ...
        def set_elements(self, elements):
            ...
            if not (isinstance(elements, tuple) and len(list(ifilterfalse(lambda _: isinstance(_, dressdiscover.api.vocabularies.costume_core.structure.structure.Structure), elements))) == 0):
                raise TypeError("expected elements to be a tuple(dressdiscover.api.vocabularies.costume_core.structure.structure.Structure) but it is a %s" % getattr(__builtin__, 'type')(elements))
            if len(elements) < 1:
        ...
        def update(self, structure_set):
            ...
            if isinstance(structure_set, StructureSet):
                self.set_elements(structure_set.elements)
                self.set_display(structure_set.display)
                self.set_notes(structure_set.notes)
            elif isinstance(structure_set, dict):
                for key, value in structure_set.iteritems():
                    getattr(self, 'set_' + key)(value)
            else:
                raise TypeError(structure_set)
            return self
    ...
    def __repr__(self):
        ...
        return 'StructureSet(' + ', '.join(field_reprs) + ')'
    ...
    def read(cls, iprot):
        ...
        while True:
            ...
            if ifield_type == 0: # STOP
                ...
                init_kwds['elements'] = tuple([dressdiscover.api.vocabularies.costume_core.structure.structure.Structure.read(iprot) for _ in xrange(iprot.read_list_begin()[1])] + (iprot.read_list_end() is None and []))
    ...
    def replace(
        ...
        :type elements: tuple(dressdiscover.api.vocabularies.costume_core.structure.structure.Structure) or None
        :type display: str or None
        :type notes: str or None
        :rtype: dressdiscover.api.vocabularies.costume_core.structure.structure_set.StructureSet
        '''
    ...
    def write(self, oprot):
        ...
        oprot.write_struct_begin('StructureSet')
