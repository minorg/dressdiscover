...
import __builtin__
import dressdiscover.api.vocabularies.vra_core.cultural_context.cultural_context


class CulturalContextSet(object):
    class Builder(object):
        def __init__(
            ...
            '''
            :type elements: tuple(dressdiscover.api.vocabularies.vra_core.cultural_context.cultural_context.CulturalContext)
        ...
        def build(self):
            return CulturalContextSet(elements=self.__elements, display=self.__display, notes=self.__notes)
        ...
        def elements(self):
            '''
            :rtype: tuple(dressdiscover.api.vocabularies.vra_core.cultural_context.cultural_context.CulturalContext)
            '''
        ...
        def set_elements(self, elements):
            ...
            if not (isinstance(elements, tuple) and len(list(ifilterfalse(lambda _: isinstance(_, dressdiscover.api.vocabularies.vra_core.cultural_context.cultural_context.CulturalContext), elements))) == 0):
                raise TypeError("expected elements to be a tuple(dressdiscover.api.vocabularies.vra_core.cultural_context.cultural_context.CulturalContext) but it is a %s" % getattr(__builtin__, 'type')(elements))
            if len(elements) < 1:
        ...
        def update(self, cultural_context_set):
            '''
            :type elements: tuple(dressdiscover.api.vocabularies.vra_core.cultural_context.cultural_context.CulturalContext)
            :type display: str or None
            :type notes: str or None
            ...
            if isinstance(cultural_context_set, CulturalContextSet):
                self.set_elements(cultural_context_set.elements)
                self.set_display(cultural_context_set.display)
                self.set_notes(cultural_context_set.notes)
            elif isinstance(cultural_context_set, dict):
                for key, value in cultural_context_set.iteritems():
                    getattr(self, 'set_' + key)(value)
            else:
                raise TypeError(cultural_context_set)
            return self
    ...
    def __repr__(self):
        ...
        return 'CulturalContextSet(' + ', '.join(field_reprs) + ')'
    ...
    def read(cls, iprot):
        ...
        while True:
            ...
            if ifield_type == 0: # STOP
                ...
                init_kwds['elements'] = tuple([dressdiscover.api.vocabularies.vra_core.cultural_context.cultural_context.CulturalContext.read(iprot) for _ in xrange(iprot.read_list_begin()[1])] + (iprot.read_list_end() is None and []))
    ...
    def replace(
        ...
        :type elements: tuple(dressdiscover.api.vocabularies.vra_core.cultural_context.cultural_context.CulturalContext) or None
        :type display: str or None
        :type notes: str or None
        :rtype: dressdiscover.api.vocabularies.vra_core.cultural_context.cultural_context_set.CulturalContextSet
        '''
    ...
    def write(self, oprot):
        ...
        oprot.write_struct_begin('CulturalContextSet')
