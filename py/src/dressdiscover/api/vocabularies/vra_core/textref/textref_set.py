...
import __builtin__
import dressdiscover.api.vocabularies.vra_core.textref.textref


class TextrefSet(object):
    class Builder(object):
        def __init__(
            ...
            '''
            :type elements: tuple(dressdiscover.api.vocabularies.vra_core.textref.textref.Textref)
        ...
        def build(self):
            return TextrefSet(elements=self.__elements, display=self.__display, notes=self.__notes)
        ...
        def elements(self):
            '''
            :rtype: tuple(dressdiscover.api.vocabularies.vra_core.textref.textref.Textref)
            '''
        ...
        def set_elements(self, elements):
            ...
            if not (isinstance(elements, tuple) and len(list(ifilterfalse(lambda _: isinstance(_, dressdiscover.api.vocabularies.vra_core.textref.textref.Textref), elements))) == 0):
                raise TypeError("expected elements to be a tuple(dressdiscover.api.vocabularies.vra_core.textref.textref.Textref) but it is a %s" % getattr(__builtin__, 'type')(elements))
            if len(elements) < 1:
        ...
        def update(self, textref_set):
            ...
            if isinstance(textref_set, TextrefSet):
                self.set_elements(textref_set.elements)
                self.set_display(textref_set.display)
                self.set_notes(textref_set.notes)
            elif isinstance(textref_set, dict):
                for key, value in textref_set.iteritems():
                    getattr(self, 'set_' + key)(value)
            else:
                raise TypeError(textref_set)
            return self
    ...
    def __repr__(self):
        ...
        return 'TextrefSet(' + ', '.join(field_reprs) + ')'
    ...
    def read(cls, iprot):
        ...
        while True:
            ...
            if ifield_type == 0: # STOP
                ...
                init_kwds['elements'] = tuple([dressdiscover.api.vocabularies.vra_core.textref.textref.Textref.read(iprot) for _ in xrange(iprot.read_list_begin()[1])] + (iprot.read_list_end() is None and []))
    ...
    def replace(
        ...
        :type elements: tuple(dressdiscover.api.vocabularies.vra_core.textref.textref.Textref) or None
        :type display: str or None
        :type notes: str or None
        :rtype: dressdiscover.api.vocabularies.vra_core.textref.textref_set.TextrefSet
        '''
    ...
    def write(self, oprot):
        ...
        oprot.write_struct_begin('TextrefSet')
