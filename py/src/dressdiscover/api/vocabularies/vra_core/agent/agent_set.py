...
import __builtin__
import dressdiscover.api.vocabularies.vra_core.agent.agent
...
class AgentSet(object):
    class Builder(object):
        def __init__(
            ...
            '''
            :type elements: tuple(dressdiscover.api.vocabularies.vra_core.agent.agent.Agent)
        ...
        def elements(self):
            '''
            :rtype: tuple(dressdiscover.api.vocabularies.vra_core.agent.agent.Agent)
            '''
        ...
        def set_elements(self, elements):
            ...
            if not (isinstance(elements, tuple) and len(list(ifilterfalse(lambda _: isinstance(_, dressdiscover.api.vocabularies.vra_core.agent.agent.Agent), elements))) == 0):
                raise TypeError("expected elements to be a tuple(dressdiscover.api.vocabularies.vra_core.agent.agent.Agent) but it is a %s" % getattr(__builtin__, 'type')(elements))
            if len(elements) < 1:
    ...
    def read(cls, iprot):
        ...
        while True:
            ...
            if ifield_type == 0: # STOP
                ...
                init_kwds['elements'] = tuple([dressdiscover.api.vocabularies.vra_core.agent.agent.Agent.read(iprot) for _ in xrange(iprot.read_list_begin()[1])] + (iprot.read_list_end() is None and []))
    ...
    def replace(
        ...
        :type elements: tuple(dressdiscover.api.vocabularies.vra_core.agent.agent.Agent) or None
        :type display: str or None
        :type notes: str or None
        :rtype: dressdiscover.api.vocabularies.vra_core.agent.agent_set.AgentSet
        '''
