...
import __builtin__
import dressdiscover.api.vocabularies.vra_core.measurements.measurements
...
class MeasurementsSet(object):
    class Builder(object):
        def __init__(
            ...
            '''
            :type elements: tuple(dressdiscover.api.vocabularies.vra_core.measurements.measurements.Measurements)
        ...
        def build(self):
            return MeasurementsSet(elements=self.__elements, display=self.__display, notes=self.__notes)
        ...
        def elements(self):
            '''
            :rtype: tuple(dressdiscover.api.vocabularies.vra_core.measurements.measurements.Measurements)
            '''
        ...
        def set_elements(self, elements):
            ...
            if not (isinstance(elements, tuple) and len(list(ifilterfalse(lambda _: isinstance(_, dressdiscover.api.vocabularies.vra_core.measurements.measurements.Measurements), elements))) == 0):
                raise TypeError("expected elements to be a tuple(dressdiscover.api.vocabularies.vra_core.measurements.measurements.Measurements) but it is a %s" % getattr(__builtin__, 'type')(elements))
            if len(elements) < 1:
        ...
        def update(self, measurements_set):
            '''
            :type elements: tuple(dressdiscover.api.vocabularies.vra_core.measurements.measurements.Measurements)
            :type display: str or None
            :type notes: str or None
            ...
            if isinstance(measurements_set, MeasurementsSet):
                self.set_elements(measurements_set.elements)
                self.set_display(measurements_set.display)
                self.set_notes(measurements_set.notes)
            elif isinstance(measurements_set, dict):
                for key, value in measurements_set.iteritems():
                    getattr(self, 'set_' + key)(value)
            else:
                raise TypeError(measurements_set)
            return self
    ...
    def __repr__(self):
        ...
        return 'MeasurementsSet(' + ', '.join(field_reprs) + ')'
    ...
    def read(cls, iprot):
        ...
        while True:
            ...
            if ifield_type == 0: # STOP
                ...
                init_kwds['elements'] = tuple([dressdiscover.api.vocabularies.vra_core.measurements.measurements.Measurements.read(iprot) for _ in xrange(iprot.read_list_begin()[1])] + (iprot.read_list_end() is None and []))
    ...
    def replace(
        ...
        :type elements: tuple(dressdiscover.api.vocabularies.vra_core.measurements.measurements.Measurements) or None
        :type display: str or None
        :type notes: str or None
        :rtype: dressdiscover.api.vocabularies.vra_core.measurements.measurements_set.MeasurementsSet
        '''
    ...
    def write(self, oprot):
        ...
        oprot.write_struct_begin('MeasurementsSet')
