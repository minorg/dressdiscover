...
import __builtin__
import dressdiscover.api.vocabularies.vra_core.location.location
...
class LocationSet(object):
    class Builder(object):
        def __init__(
            ...
            '''
            :type elements: tuple(dressdiscover.api.vocabularies.vra_core.location.location.Location)
        ...
        def build(self):
            return LocationSet(elements=self.__elements, display=self.__display, notes=self.__notes)
        ...
        def elements(self):
            '''
            :rtype: tuple(dressdiscover.api.vocabularies.vra_core.location.location.Location)
            '''
        ...
        def set_elements(self, elements):
            ...
            if not (isinstance(elements, tuple) and len(list(ifilterfalse(lambda _: isinstance(_, dressdiscover.api.vocabularies.vra_core.location.location.Location), elements))) == 0):
                raise TypeError("expected elements to be a tuple(dressdiscover.api.vocabularies.vra_core.location.location.Location) but it is a %s" % getattr(__builtin__, 'type')(elements))
            if len(elements) < 1:
        ...
        def update(self, location_set):
            ...
            if isinstance(location_set, LocationSet):
                self.set_elements(location_set.elements)
                self.set_display(location_set.display)
                self.set_notes(location_set.notes)
            elif isinstance(location_set, dict):
                for key, value in location_set.iteritems():
                    getattr(self, 'set_' + key)(value)
            else:
                raise TypeError(location_set)
            return self
    ...
    def __repr__(self):
        ...
        return 'LocationSet(' + ', '.join(field_reprs) + ')'
    ...
    def read(cls, iprot):
        ...
        while True:
            ...
            if ifield_type == 0: # STOP
                ...
                init_kwds['elements'] = tuple([dressdiscover.api.vocabularies.vra_core.location.location.Location.read(iprot) for _ in xrange(iprot.read_list_begin()[1])] + (iprot.read_list_end() is None and []))
    ...
    def replace(
        ...
        :type elements: tuple(dressdiscover.api.vocabularies.vra_core.location.location.Location) or None
        :type display: str or None
        :type notes: str or None
        :rtype: dressdiscover.api.vocabularies.vra_core.location.location_set.LocationSet
        '''
    ...
    def write(self, oprot):
        ...
        oprot.write_struct_begin('LocationSet')
