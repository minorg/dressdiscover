...
import __builtin__
import dressdiscover.api.vocabularies.vra_core.technique.technique
...
class TechniqueSet(object):
    class Builder(object):
        def __init__(
            ...
            '''
            :type elements: tuple(dressdiscover.api.vocabularies.vra_core.technique.technique.Technique)
        ...
        def build(self):
            return TechniqueSet(elements=self.__elements, display=self.__display, notes=self.__notes)
        ...
        def elements(self):
            '''
            :rtype: tuple(dressdiscover.api.vocabularies.vra_core.technique.technique.Technique)
            '''
        ...
        def set_elements(self, elements):
            ...
            if not (isinstance(elements, tuple) and len(list(ifilterfalse(lambda _: isinstance(_, dressdiscover.api.vocabularies.vra_core.technique.technique.Technique), elements))) == 0):
                raise TypeError("expected elements to be a tuple(dressdiscover.api.vocabularies.vra_core.technique.technique.Technique) but it is a %s" % getattr(__builtin__, 'type')(elements))
            if len(elements) < 1:
        ...
        def update(self, technique_set):
            ...
            if isinstance(technique_set, TechniqueSet):
                self.set_elements(technique_set.elements)
    ...
    def read(cls, iprot):
        ...
        while True:
            ...
            if ifield_type == 0: # STOP
                ...
                init_kwds['elements'] = tuple([dressdiscover.api.vocabularies.vra_core.technique.technique.Technique.read(iprot) for _ in xrange(iprot.read_list_begin()[1])] + (iprot.read_list_end() is None and []))
    ...
    def replace(
        ...
        :type elements: tuple(dressdiscover.api.vocabularies.vra_core.technique.technique.Technique) or None
        :type display: str or None
        :type notes: str or None
        :rtype: dressdiscover.api.vocabularies.vra_core.technique.technique_set.TechniqueSet
        '''
