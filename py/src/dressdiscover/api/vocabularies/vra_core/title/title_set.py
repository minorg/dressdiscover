...
import __builtin__
import dressdiscover.api.vocabularies.vra_core.title.title
...
class TitleSet(object):
    class Builder(object):
        def __init__(
            ...
            '''
            :type elements: tuple(dressdiscover.api.vocabularies.vra_core.title.title.Title)
        ...
        def build(self):
            return TitleSet(elements=self.__elements, display=self.__display, notes=self.__notes)
        ...
        def elements(self):
            '''
            :rtype: tuple(dressdiscover.api.vocabularies.vra_core.title.title.Title)
            '''
        ...
        def set_elements(self, elements):
            ...
            if not (isinstance(elements, tuple) and len(list(ifilterfalse(lambda _: isinstance(_, dressdiscover.api.vocabularies.vra_core.title.title.Title), elements))) == 0):
                raise TypeError("expected elements to be a tuple(dressdiscover.api.vocabularies.vra_core.title.title.Title) but it is a %s" % getattr(__builtin__, 'type')(elements))
            if len(elements) < 1:
        ...
        def update(self, title_set):
            ...
            if isinstance(title_set, TitleSet):
                self.set_elements(title_set.elements)
                self.set_display(title_set.display)
                self.set_notes(title_set.notes)
            elif isinstance(title_set, dict):
                for key, value in title_set.iteritems():
                    getattr(self, 'set_' + key)(value)
            else:
                raise TypeError(title_set)
            return self
    ...
    def __repr__(self):
        ...
        return 'TitleSet(' + ', '.join(field_reprs) + ')'
    ...
    def read(cls, iprot):
        ...
        while True:
            ...
            if ifield_type == 0: # STOP
                ...
                init_kwds['elements'] = tuple([dressdiscover.api.vocabularies.vra_core.title.title.Title.read(iprot) for _ in xrange(iprot.read_list_begin()[1])] + (iprot.read_list_end() is None and []))
    ...
    def replace(
        ...
        :type elements: tuple(dressdiscover.api.vocabularies.vra_core.title.title.Title) or None
        :type display: str or None
        :type notes: str or None
        :rtype: dressdiscover.api.vocabularies.vra_core.title.title_set.TitleSet
        '''
    ...
    def write(self, oprot):
        ...
        oprot.write_struct_begin('TitleSet')
