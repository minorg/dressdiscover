...
class WorkType(object):
    class Builder(object):
        ...
        def build(self):
            return WorkType(text=self.__text, vocab_ref=self.__vocab_ref)
        ...
        def update(self, work_type):
            ...
            if isinstance(work_type, WorkType):
                self.set_text(work_type.text)
                self.set_vocab_ref(work_type.vocab_ref)
            elif isinstance(work_type, dict):
                for key, value in work_type.iteritems():
                    getattr(self, 'set_' + key)(value)
            else:
                raise TypeError(work_type)
            return self
    ...
    def __repr__(self):
        ...
        return 'WorkType(' + ', '.join(field_reprs) + ')'
    ...
    def read(cls, iprot):
        ...
        :rtype: dressdiscover.api.vocabularies.vra_core.work_type.work_type.WorkType
        '''
