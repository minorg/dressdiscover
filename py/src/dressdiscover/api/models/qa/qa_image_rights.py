...
class QaImageRights(object):
    class Builder(object):
        ...
        def build(self):
            return QaImageRights(author=self.__author, license=self.__license, source_name=self.__source_name, source_url=self.__source_url)
        ...
        def update(self, qa_image_rights):
            '''
            :type author: str
            :type license: str
            :type source_name: str
            ...
            if isinstance(qa_image_rights, QaImageRights):
                self.set_author(qa_image_rights.author)
                self.set_license(qa_image_rights.license)
                self.set_source_name(qa_image_rights.source_name)
                self.set_source_url(qa_image_rights.source_url)
            elif isinstance(qa_image_rights, dict):
                for key, value in qa_image_rights.iteritems():
                    getattr(self, 'set_' + key)(value)
            else:
                raise TypeError(qa_image_rights)
            return self
    ...
    def __repr__(self):
        ...
        return 'QaImageRights(' + ', '.join(field_reprs) + ')'
    ...
    def write(self, oprot):
        ...
        oprot.write_struct_begin('QaImageRights')
