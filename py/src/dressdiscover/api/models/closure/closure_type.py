...
class ClosureType(object):
    class Builder(object):
        ...
        def build(self):
            return ClosureType(text=self.__text, vocab_ref=self.__vocab_ref)
        ...
        def update(self, closure_type):
            '''
            :type text: str
            :type vocab_ref: costume.api.models.vocab_ref.VocabRef or None
            '''

            if isinstance(closure_type, ClosureType):
                self.set_text(closure_type.text)
                self.set_vocab_ref(closure_type.vocab_ref)
            elif isinstance(closure_type, dict):
                for key, value in closure_type.iteritems():
                    getattr(self, 'set_' + key)(value)
            else:
                raise TypeError(closure_type)
            return self
    ...
    def __repr__(self):
        ...
        return 'ClosureType(' + ', '.join(field_reprs) + ')'
    ...
    def write(self, oprot):
        ...
        oprot.write_struct_begin('ClosureType')
