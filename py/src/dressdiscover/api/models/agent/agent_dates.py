...
class AgentDates(object):
    ...
    def __init__(
        ...
        if earliest_date is not None:
            if not isinstance(earliest_date, datetime):
                raise TypeError("expected earliest_date to be a datetime but it is a %s" % getattr(__builtin__, 'type')(earliest_date))
        self.__earliest_date = earliest_date

        if latest_date is not None:
            if not isinstance(latest_date, datetime):
                raise TypeError("expected latest_date to be a datetime but it is a %s" % getattr(__builtin__, 'type')(latest_date))
        self.__latest_date = latest_date
