...
class ObjectStoreConfiguration(object):
    class Builder(object):
        ...
        def build(self):
            return ObjectStoreConfiguration(type=self.__type, parameters=self.__parameters)
        ...
        def update(self, object_store_configuration):
            '''
            :type type: str
            :type parameters: dict(str: str) or None
            '''

            if isinstance(object_store_configuration, ObjectStoreConfiguration):
                self.set_type(object_store_configuration.type)
                self.set_parameters(object_store_configuration.parameters)
            elif isinstance(object_store_configuration, dict):
                for key, value in object_store_configuration.iteritems():
                    getattr(self, 'set_' + key)(value)
            else:
                raise TypeError(object_store_configuration)
            return self
    ...
    def __repr__(self):
        ...
        return 'ObjectStoreConfiguration(' + ', '.join(field_reprs) + ')'
    ...
    def write(self, oprot):
        ...
        oprot.write_struct_begin('ObjectStoreConfiguration')
