...
class Collection(object):
    class Builder(object):
        ...
        def build(self):
            return Collection(title=self.__title, description=self.__description, external=self.__external, hidden=self.__hidden, locations=self.__locations, url=self.__url, work_types=self.__work_types)
    ...
    class FieldMetadata(object):
        ...
        @classmethod
        def values(cls):
            return (cls.TITLE, cls.DESCRIPTION, cls.EXTERNAL, cls.HIDDEN, cls.LOCATIONS, cls.URL, cls.WORK_TYPES,)
    ...
    def __hash__(self):
        return hash((self.title,self.description,self.external,self.hidden,self.locations,self.url,self.work_types,))

    def __iter__(self):
        return iter((self.title, self.description, self.external, self.hidden, self.locations, self.url, self.work_types,))
    ...
    def replace(
        ...
        return self.__class__(title=title, description=description, external=external, hidden=hidden, locations=locations, url=url, work_types=work_types)
