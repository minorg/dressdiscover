...
class AgentRole(object):
    class Builder(object):
        ...
        def build(self):
            return AgentRole(text=self.__text, refid=self.__refid, vocab=self.__vocab)
        ...
        def update(self, agent_role):
            ...
            if isinstance(agent_role, AgentRole):
                self.set_text(agent_role.text)
                self.set_refid(agent_role.refid)
                self.set_vocab(agent_role.vocab)
            elif isinstance(agent_role, dict):
                for key, value in agent_role.iteritems():
                    getattr(self, 'set_' + key)(value)
            else:
                raise TypeError(agent_role)
            return self
    ...
    def write(self, oprot):
        ...
        oprot.write_struct_begin('AgentRole')
