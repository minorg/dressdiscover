...
from paradicms_etl.transformers._csv_file_transformer import _CsvFileTransformer
    ...
    class __SchccObject(Object):
        def __init__(self, *, csv_row: Dict[str, str], graph: Graph):
            ...
            self.__transform(
                ...
                property=URIRef(costume_core_predicates.collar.uri),
            ...
            self.__transform(
                multi=False, csv_column_key="Creator", property=DCTERMS.creator
            )
            self.__transform(
                multi=True,
            ...
            self.__transform(
                multi=False, csv_column_key="Description", property=DCTERMS.description
            )
            self.__csv_row.pop(
                "Description Autofill", None
            ...
            )  # Handled in collection/institution transform
            self.__transform(
                multi=False, csv_column_key="Identifier", property=DCTERMS.identifier
            )
            # Skip Label
            self.__transform(
                multi=True,
            ...
            )  # Ignore for now, no predicates
            self.__csv_row.pop(
                "Measurements Autofill", None
            )  # Created from other columns
            self.__transform(
                multi=True, csv_column_key="Medium", property=DCTERMS.medium
            )
            self.__transform(
                multi=False,
            ...
            self.__transform(
                multi=False, csv_column_key="Subject", property=DCTERMS.subject
            )
            self.__transform(
                multi=False, csv_column_key="Title", property=DCTERMS.title
            )
            ...
            self.__transform(
                ...
                costume_core_terms=COSTUME_CORE_TERMS_BY_FEATURES["workType"],
            )  # CC also has a workType
            ...
            for csv_row in csv.DictReader(csv_file):
                ...
                if institution is None:
                    ...
                    institution = Institution(
                        graph=graph, uri=URIRef("http://www.smith.edu/")
                    )
                    institution.name = "Smith College"
                    institution.owner = CMS.public

                    collection = Collection(
                        graph=graph, uri=URIRef("http://schistoricclothing.com/")
                    )
                    ...
                    collection.resource.add(
                        DCTERMS.rights, Literal(csv_row["Rights Holder"])
                    )
                    collection.resource.add(
